    ...
    pub unsafe fn new(inner: *mut rust_htslib::htslib::bcf_hdr_t) -> Self {
        ...
        for record in view.header_records() {
            let tag_id = match &record {
                HeaderRecord::Info { values, .. } | HeaderRecord::Format { values, .. } => values
                    .iter()
        ...
        for record in self.header_records() {
            match record {
                HeaderRecord::Info { values, .. } => {
                    if let Some(field) =
                        self.parse_record_to_field("INFO", values.into_iter().collect())
                    {
                ...
                HeaderRecord::Format { values, .. } => {
                    if let Some(field) =
                        self.parse_record_to_field("FORMAT", values.into_iter().collect())
                    {
                ...
                HeaderRecord::Filter { values, .. } => {
                    if let Some(field) =
                        self.parse_record_to_field("FILTER", values.into_iter().collect())
                    {
