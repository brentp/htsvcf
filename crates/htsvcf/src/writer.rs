...
fn parse_options(scope: &mut v8::PinScope<'_, '_>, v: v8::Local<v8::Value>) -> Option<WriterOptions> {
...
fn writer_ctor(
  ...
  let Some(header_wrapper) = (unsafe { v8::Object::unwrap::<{ crate::header::HEADER_TAG }, Header>(scope, header_obj) }) else {
    throw_type_error(scope, "Writer(path, header): header must be Header");
    return;
  };
  ...
  let opts = if args.length() >= 3 {
    match parse_options(scope, args.get(2)) {
      Some(o) => o,
      None => {
        throw_type_error(scope, "Writer options must be {format?, uncompressed?, threads?}");
        return;
      }
    }
  } else {
  ...
  let gc_wrapper = unsafe { v8::cppgc::make_garbage_collected(scope.get_cpp_heap().unwrap(), wrapper) };
  unsafe {
...
fn writer_close_fn(
  ...
  let this = args.this();
  let Some(wrapper_ptr) = (unsafe { v8::Object::unwrap::<WRITER_TAG, WriterWrapper>(scope, this) }) else {
...
pub(crate) fn create_writer_constructor<'a>(
  ...
  proto.set(write_key.into(), v8::FunctionTemplate::new(scope, writer_write_fn).into());

  let close_key = v8::String::new(scope, "close").unwrap();
  proto.set(close_key.into(), v8::FunctionTemplate::new(scope, writer_close_fn).into());
