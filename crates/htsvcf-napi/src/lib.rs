...
impl Reader {
    ...
    pub fn new(env: Env, path: String, _opts: Option<ReaderOptions>) -> napi::Result<Self> {
        let reader = core::open_reader(&path).map_err(|e| {
            Error::new(
        ...
        let header_ref = Header::into_reference(
            Header {
                inner: header.clone(),
            },
            env,
...
impl Writer {
    ...
    pub fn new(
        ...
        let writer = core::open_writer(&path, header.inner.as_ref(), options).map_err(|e| {
            Error::new(
                Status::GenericFailure,
                format!("failed to open writer: {e}"),
            )
        })?;
...
impl Task for OpenReaderTask {
    ...
    fn compute(&mut self) -> napi::Result<Self::Output> {
        core::open_reader(&self.path).map_err(|e| {
            Error::new(
                Status::GenericFailure,
                format!("failed to open {}: {e}", self.path),
            )
        })
    }
...
impl Variant {
    ...
    pub fn sample(&self, env: Env, name: String) -> napi::Result<sys::napi_value> {
        ...
        for (tag, value) in fields {
            ...
            out.set_named_property(tag.as_str(), unsafe {
                Unknown::from_raw_unchecked(env.raw(), js_value)
            })?;
        }
    ...
    pub fn samples(&self, env: Env, subset: Option<Vec<String>>) -> napi::Result<sys::napi_value> {
        ...
        let all_samples = self
            .variant()?
            .samples(&self.header, subset_refs.as_deref());
    ...
    pub fn set_genotypes(&mut self, env: Env, genotypes: Array) -> napi::Result<()> {
        ...
        self.variant_mut()?.set_genotypes(&gts).map_err(|e| {
            Error::new(
                Status::GenericFailure,
                format!("failed to set genotypes: {e}"),
            )
        })
    }
    ...
    pub fn set_info(&mut self, tag: String, value: Unknown) -> napi::Result<()> {
        ...
        match value.get_type()? {
            ValueType::Null | ValueType::Undefined => {
                self.variant_mut()?.clear_info(&header, &tag).map_err(|e| {
                    Error::new(
                        Status::GenericFailure,
                        format!("failed to clear info {tag}: {e}"),
                    )
                })?;
                return Ok(());
            }
...
fn infovalue_to_napi_value(env: &Env, v: &core::InfoValue) -> napi::Result<sys::napi_value> {
    match v {
        ...
        core::InfoValue::Int(i) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_int32(*i)?)
        },
        core::InfoValue::Float(f) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_double(*f as f64)?)
        },
        core::InfoValue::String(s) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_string(s)?)
        },
...
fn formatvalue_to_napi_value(env: &Env, v: &FormatValue) -> napi::Result<sys::napi_value> {
    match v {
        ...
        FormatValue::Int(i) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_int32(*i)?)
        },
        FormatValue::Float(f) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_double(*f as f64)?)
        },
        FormatValue::String(s) => unsafe {
            ToNapiValue::to_napi_value(env.raw(), env.create_string(s)?)
        },
